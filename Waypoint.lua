...
function Course:enrichWaypointData(forceAngleRecalculation)
	...
	for i = 1, #self.waypoints - 1 do
		...
		if not self.waypoints[i].angle or forceAngleRecalculation then
...
--- Use this when the course was generated by turn.lua to fix:
--- 1. waypoints too close to each other or even on top of each other
--- 2. direction changes
-- TODO: fix turn.lua to generate proper courses
function Course:fixCourseGeneratedByTurnLua()
	...
	self:enrichWaypointData(true)
	self:print()
	local reverse = self.waypoints[1].rev
